...
import { layoutTextLayer, layerSize, layerAABB } from "./render";
...
// box width changes (left-aligned text keeps its left edge, etc).
export function preserveAnchorX(oldLayer, newLayer) {
  ...
  if (newLayer.align === "left") return { ...newLayer, x: newLayer.x - oldW / 2 + newW / 2 };
  if (newLayer.align === "right") return { ...newLayer, x: newLayer.x + oldW / 2 - newW / 2 };
  return newLayer;
}
