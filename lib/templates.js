...
import { BRANDS, BLOCK_PRESETS, TEMPLATES, CANVAS_SIZES, remapColor } from "./constants";
...
export function stackMembers(layers, stackId) {
  return layers
    .filter((l) => l.stack && l.stack.id === stackId && l.type === "text" && l.visible !== false)
...
export function createDefaultDoc(brand = "Estus") {
  ...
  const canvas = { preset, ...CANVAS_SIZES[preset], background: BRANDS[brand].defaultBg };
  return {
    brand,
    canvas,
