...
// a repaint when a newly referenced image finishes decoding.
const imageCache = new Map();
export function getImage(src, onLoad) {
  let entry = imageCache.get(src);
  if (!entry) {
    const img = new Image();
    entry = { img, loaded: false, failed: false, callbacks: [] };
...
function filterKey(layer, scale) {
  const f = layer.filters || {};
  return [
    layer.src.length, layer.src.slice(-32), Math.round(layer.w), Math.round(layer.h),
...
// (posX/posY, 0-100, used by cover cropping and contain placement); bleed
// expands the dest rect on all sides (caller clips).
export function drawImageFitRect(ctx, img, dx, dy, dw, dh, fit, posX = 50, posY = 50, bleed = 0) {
  ...
  let ox = dx - bleed, oy = dy - bleed, ow = dw + bleed * 2, oh = dh + bleed * 2;
  ...
  if (fit === "cover") {
    ...
    if (imgRatio > boxRatio) {
      ...
      oy = dy + ((dh - drawH) * posY) / 100 - bleed;
      oh = drawH + bleed * 2;
    } else {
      const drawW = dh * imgRatio;
      ox = dx + ((dw - drawW) * posX) / 100 - bleed;
      ow = drawW + bleed * 2;
    }
  }
...
export async function exportDesign(doc, { pixelRatio = 1, format = "png", quality = 0.92, transparentBg = false } = {}) {
  ...
  canvas.width = Math.round(doc.canvas.w * ratio);
  canvas.height = Math.round(doc.canvas.h * ratio);
  const ctx = canvas.getContext("2d");
  renderDesign(ctx, doc, { scale: ratio, transparentBg: transparentBg && format === "png" });
